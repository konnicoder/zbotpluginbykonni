        ...
        try {
            ...
            while (concretemined < neededpowder && checkToolHealth() == true && abbruch == false) {
                ...
                if (checkToolHealth() == false) {
                    Main.self.sendChat("my tool needs repairing, daddy");
                    healTool();
                    ai.tick();
                    dumpTrash();
                    ai.tick();
        ...
        if (KaiTools.testStackAvaliable(colourpowder, powderslot) == false) {
            if (KaiTools.lookInInventoryAndMove(colourpowder, powderslot, ai) == false) {
                if (getPowderFromChest() == false) {
                    ...
                    //craftConcretPowder(craftingrecipe);
                    abbruch = true;
                    return false;
                    //false und abbruch == true
                }
            }
        }
        return true;
...
//checks if powder is in definded powderchest and if so, gets 1 stack
    public boolean getPowderFromChest() throws InterruptedException {
        powdertoget = neededpowder - concretemined;
    ...
    public boolean getDyeFromChest() throws InterruptedException {
        ai.moveTo(DYE_CHEST_WALK);
        ai.tick();
        ai.openContainer(DYE_CHEST_LOC);

        if (KaiTools.lookInDoubleChestAndMove(colourdye, 38, ai) == false) {
            ai.closeContainer();
            abbruch = true;
            Main.self.sendChat("cant find any fitting dye");
            return false;
        }
        ai.closeContainer();
        return true;
    ...
    public void mineConcrete() throws InterruptedException {
        ...
        ai.tick();
        Main.self.selectSlot(0);
        ai.breakBlock(CONCRETE_FARM_LOC, 10);
        concretemined++;
    }

    public void craftConcretPowder(String recipestring) throws InterruptedException {
        ai.moveTo(GRAVEL_TESSERACT_WALK);
        ai.tick();
        while (InventoryUtil.countFullStacks(Material.GRAVEL, 9, 44) < 4) {
            ai.clickBlock(GRAVEL_TESSERACT_LOC);
            ai.tick();
        }
        ai.tick(3);
        ai.moveTo(SAND_TESSERACT_WALK);
        while (InventoryUtil.countFullStacks(Material.SAND, 9, 44) < 4) {
            ai.clickBlock(SAND_TESSERACT_LOC);
            ai.tick();
        }
        ai.tick(3);
        getDyeFromChest();
        ai.moveTo(POWDER_CRAFTING_LOC);
        ai.tick();
        ai.openContainer(POWDER_CRAFTING_LOC);
        ai.tick();
        Main.self.recipeBookStatus(true, false, true, false, true, false, true, false);
        Main.self.requestRecipe(recipestring, true);
