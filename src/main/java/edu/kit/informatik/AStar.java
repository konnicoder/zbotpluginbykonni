...
public class AStar {
    ...
    public static final GeometricPath getPath(Location target, boolean returnPartial) {
        ...
        visited = new HashSet<>();
        Location myLocation = Main.self.getLocation().centerHorizontally();

        GeometricPath root = new GeometricPath(myLocation);
        searchPerimeter.add(root);
        visited.add(Main.self.getLocation().toLong());
        ...
        while (true) {
            ...
            if (!isVisited(x, y + 1, z)
                    && isBlockFree(x, y + 2, z)
                    && Main.self.getEnvironment().getBlockAt(x, y, z).getType() == Material.LADDER) {
                /*
            ...
            if (!isVisited(x, y - 1, z)
                    && isBlockFree(x, y - 1, z)
                    && Main.self.getEnvironment().getBlockAt(x, y - 1, z).getType() == Material.LADDER) {
                /*
    ...
    private static final boolean isBlockWalkable(int x, int y, int z) {
        if (Main.self.getEnvironment().getBlockAt(x, y, z).getType() == Material.LADDER) {
            return true;
        }
        return !isBlockFree(x, y, z);// && !FORBIDDEN_BLOCKS.contains(Main.self.getEnvironment().getBlockAt(x, y, z).getTypeId());
    }

    private static final boolean isBlockFree(int x, int y, int z) {
        return !Main.self.getEnvironment().getBlockAt(x, y, z).getType().isSolid(); //NONSOLID_BLOCKS.contains(Main.self.getEnvironment().getBlockAt(x, y, z).getTypeId());
    }
